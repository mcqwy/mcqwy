...
public class BlockAssembling extends BlockContainer
{
	public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);
	public static final PropertyBool BURNING = PropertyBool.create("burning");

	public BlockAssembling()
    {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
		if (!worldIn.isRemote)
        {
			int id = GuiElementLoader.GUI_ASS_ASS;
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        if (tile instanceof TileEntityAssembling)
        {
            ...
            IItemHandler west = tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, EnumFacing.WEST);
            ...
            for (int i = west.getSlots() - 1; i >= 0; --i)
            {
                if (west.getStackInSlot(i) != null)
                {
                    Block.spawnAsEntity(worldIn, pos, west.getStackInSlot(i));
                }
            }
        }
        Block.spawnAsEntity(worldIn, pos, new ItemStack(BlockLoader.assembling));
	...
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new TileEntityAssembling();
	}
